...
const SalesHistory = () => {
  ...
  const [seeFeedBack, setSeeFeedack] = useState(false);
  ...
  useEffect(() => {
    api
      .get("/admin/get-completed-orders")
      .then((res) => {
  ...
  useEffect(() => {
    ...
    const FilterSalesThisDay = (db_dates, array) => {
      const filterArray = array.filter(
        (_, index) => db_dates[index] === format(currentDate, pattern)
      );
    ...
    if (selectedOption === "All") {
      ...
      const salesThiDay = completedOrders.map((completedOrder) => ({
        ...
        Orders: {
          Date: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.Date
          ),
          AddsOn: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.AddsOn
          ),
          MOD: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.MOD
          ),
          MTname: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.MTname
          ),
          Price: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.Price
          ),
          Qty: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.Qty
          ),
          Size: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.Size
          ),
          Status: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.Status
          ),
          Total: FilterSalesThisDay(
            completedOrder.Orders.Date,
            completedOrder.Orders.Total
          ),
          FeedBack: FilterSalesThisDay(
            completedOrder.Orders.FeedBack,
            completedOrder.Orders.FeedBack
          ),
        },
      }));
      ...
      const salesThisWeek = completedOrders.map((completedOrder) => ({
        ...
        Lname: completedOrder.Lname,
        Orders: {
          Date: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.Date
          ),
          AddsOn: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.AddsOn
          ),
          MOD: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.MOD
          ),
          MTname: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.MTname
          ),
          Price: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.Price
          ),
          Qty: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.Qty
          ),
          Size: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.Size
          ),
          Status: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.Status
          ),
          Total: FilterSalesThisWeek(
            completedOrder.Orders.Date,
            completedOrder.Orders.Total
          ),
          FeedBack: FilterSalesThisDay(
      ...
      const salesThisMonth = completedOrders.map((completedOrder) => ({
        ...
        Lname: completedOrder.Lname,
        Orders: {
          Date: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.Date
          ),
          AddsOn: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.AddsOn
          ),
          MOD: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.MOD
          ),
          MTname: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.MTname
          ),
          Price: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.Price
          ),
          Qty: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.Qty
          ),
          Size: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.Size
          ),
          Status: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.Status
          ),
          Total: FilterSalesThisMonth(
            completedOrder.Orders.Date,
            completedOrder.Orders.Total
          ),
          FeedBack: FilterSalesThisDay(
  ...
  const viewFeedBack = (feedBack, customerName) => {
    // alert(feedBack);
  ...
  return (
    <div>
      ...
      <div className="p-4 h-full w-full sm:w-full bg-gray-200 mx-auto">
        <div className="mb-4 text-amber-950 mb-4 flex justify-center "></div>

        {/* Display sales data based on the selected option */}
        <div className="bg-white p-4 rounded-lg shadow-lg w-full mx-auto">
          <h2 className="text-3xl text-amber-950 font-bold mb-4 flex justify-center ">
            Sales History
          </h2>
          <label className="block text-sm font-semibold mb-2 whitespace-nowrap md:ml-0 sm:-ml-10 ">
            Filter Sales:
          </label>
                    ...
                    return (
                      <tr key={inner_index}>
                        ...
                        <td className="border p-2">
                          {sale.Orders.Date[inner_index]}
                        </td>
                        <td className="border p-2">
                          {sale.Orders.MTname[inner_index]}
                        </td>
                        <td className="border p-2">
                          {sale.Orders.AddsOn[inner_index]}
                        </td>
                        <td className="border p-2">
                          {sale.Orders.Qty[inner_index]}
                        </td>
                        <td className="border p-2">
                          {sale.Orders.Size[inner_index]}
                        </td>
                        <td className="border p-2">
                          {sale.Orders.Price[inner_index]}
                        </td>
                        <td className="border p-2">
                          {sale.Orders.Total[inner_index]}
                        </td>
                        <td className="border p-2">
                          {sale.Orders.MOD[inner_index]}
                        </td>
                        <td className="border p-2">
                          <button
                            className="w-30 bg-lime-800 text-white p-3 md:hover:scale-110 rounded-xl hover:bg-white transition duration-300 relative bg-amber-950 isolation-auto z-10 border-lime-500 before:absolute before:w-full before:transition-all before:duration-700 before:hover:w-full before:-left-full before:hover:left-0 before:rounded-full before:bg-amber-950 before:-z-10 before:aspect-square before:hover:scale-150 overflow-hidden before:hover:duration-700"
                            onClick={()=> viewFeedBack(sale.Orders.FeedBack[inner_index], (sale.Fname + " " + sale.Lname))}
                          >
                            View Feedback
                          </button>
                        </td>
