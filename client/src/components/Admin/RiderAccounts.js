...
function RiderAccounts() {
  ...
  const [raidersInfo, setRaiderInfo] = useState([]);
  ...
  const deleteRaider = (ids) => {
    ...
    if(result) {
      api.post('/admin/delete-raider', ids)
      .then(res => {
        if(res.status === 200){
            ...
            {raidersInfo.map((raiderInfo) => {
                return (
                  <Fragment key={raiderInfo._id}> 
                    <tr>
                      <td className="border border-amber-900 p-2">{raiderInfo._id}</td>
                      ...
                      <td className="border border-amber-900 p-2">{raiderInfo.Uname}</td>
                      <td className="border border-amber-900 p-2">
                        {raiderInfo.Fname}
                      </td>
                      <td className="border border-amber-900 p-2">{raiderInfo.Mname}</td>
                      <td className="border border-amber-900 p-2">{raiderInfo.Lname}</td>
